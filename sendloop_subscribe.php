...
define('SENDLOOP_SUBSCRIBE', '0.3.0');
...
$pluginOptions = array(
  ...
  'sl_login'          => __('You login at Sendloop.com'),
  'sl_password'       => __('Password'),
...
class SendloopSubscribeFront {
  ...
  static function draw ($data) {
    ...
    $html = '
      <form role="slSubscribeForm" method="post" id="slSubscribeForm'.$widgetSessionKey.'" action="'.SL_SUBSCRIBE_ACTION.'">
        ...
        <div id="slResultContainer"></div>
    	  <div>
  	      <p>
	          <label for="slSubscriptionField'.$widgetSessionKey.'">Email:</label><br />
    ...
    $html .= '</div>';
    $html .= '<p>
  	        <input type="submit" id="slSubmitSubscription'.$widgetSessionKey.'" value="'.__('Submit').'">
    	    </p>
      	</div>
      </form>';
    $js = "
...
class SendloopSubscribeDispatcher {
  ...
  private function _getCommandUrl ($command, $params = array()) {
    if (!$this->_SessionID) {
      $url = $this->_options['api_url'].sprintf(
        'Command=User.Login&APIKey=%s&Username=%s&Password=%s&ResponseFormat=JSON', 
      ...
      if (true == $response['Success'])
        ...
        $this->_setResponse(__('Sendloop credentials are incorrect'));
        $this->_sendResponse();
      }
    }
  ...
  public function init ($testData = array()) {
    ...
    $this->_options['api_url'] = rtrim($this->_options['account_url'], '/').'/api/v2/';
    return $this->_options;
  }
  
  public function getSubscriberLists () {
    return $this->_getResponse($this->_getCommandUrl('Lists.Get'));
  }
  ...
  public function getSubscriberListFields ($listId) {
    ...
    try {
      $params = array(
        'ListID'              => $this->_targetListID,
        'ReturnCustomFields'  => true
      );
      $response = $this->_getResponse($this->_getCommandUrl('List.Get', $params));
      if ($response['Success']) {
        $result = $response['List']['CustomFields'];
      }
    } catch (Exception $e) {}
    return $result;
  }
